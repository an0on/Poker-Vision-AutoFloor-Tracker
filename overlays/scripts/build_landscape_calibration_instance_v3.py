...
svg_path = Path('/opt/data/tmp/poker_table_calibration_instance_current_table_v3_landscape.svg')
png_path = Path('/opt/data/tmp/poker_table_calibration_instance_current_table_v3_landscape.png')
json_path = Path('/opt/data/tmp/poker_table_calibration_instance_current_table_v3_landscape.json')
...
def capsule_path(points):
    tl=points['top_left']; tr=points['top_right']; bl=points['bottom_left']
    r=(bl[1]-tl[1])/2
    top_y=tl[1]
    bot_y=bl[1]
    left_cx=tl[0]
...
for seat_id, left_div, right_div in seat_order:
    ...
    poly = [a['outer_ring_point'], b['outer_ring_point'], b['inner_ring_point'], a['inner_ring_point']]
...
instance = {
    'schema_version': '1.2',
    'table_model_id': 'dopo_10max_calibration_instance_current_table_v3_landscape',
    'source_image': str(src),
    'image': {'width': W, 'height': H, 'orientation': 'landscape'},
    'global_geometry': {
        'outer_rail': {'kind': 'capsule_from_4_points', 'points': outer, 'points_normalized': {k: norm(v) for k, v in outer.items()}},
        'inner_rail': {'kind': 'capsule_from_4_points', 'points': inner, 'points_normalized': {k: norm(v) for k, v in inner.items()}},
        'action_area': {'kind': 'capsule_from_4_points', 'points': action, 'points_normalized': {k: norm(v) for k, v in action.items()}},
    },
    'seat_dividers': {
        'count': 10,
        'lines': {
    ...
    'dealer_button_tracking': {
        ...
        'notes': [
            'Dealer button is assumed to be visually distinctive and directly detectable.',
            'After detection, assign it to the nearest player area / seat wedge.',
            'No dedicated dealer-button boundary band is required in this calibration.'
        ]
    },
    'table_metadata': {
...
parts.append('<text class="seat1" x="585" y="170">Seat 1 default = top center wedge</text>')
parts.append('</svg>')
