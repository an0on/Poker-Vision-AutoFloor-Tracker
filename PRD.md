...
- REQ-10a: Interaktives Klick-Tool (`calib mark-zones`) zum Authoring der Referenzfoto-Geometrie, statt Koordinaten von Hand zu tippen oder algorithmisch zu raten (automatische Farbsegmentierung ist bei diesem Tischdesign nachweislich unzuverlässig — benachbarte gleichfarbige Sitze sind nur durch eine schwache Naht statt einer kontrastreichen Linie getrennt). Nimmt ein beliebiges Bild entgegen (Referenzfoto oder frisch aufgenommener Live-Frame — dasselbe Werkzeug für beide Fälle):
  ...
  - Operator zeichnet `dealer_area` (= Fläche innerhalb des inneren Ovals, die "Action Area") als freihändige Punktfolge entlang der tatsächlich aufgedruckten inneren Kurve nach — beliebig viele Punkte, im selben Stil wie beim Nachzeichnen der äußeren Rail-Kurve eines Sitzes, per Enter/Space abgeschlossen. **Nicht** das ursprüngliche 3-Punkt-Bogen-Design (Bogenanfang, Kreismittelpunkt, Bogenende): der Kreismittelpunkt-Klick erwies sich als fehleranfällig — ein leicht falsch platzierter Mittelpunkt erzeugt einen beliebig falschen Radius, ohne dass der Fehler vor dem Speichern sichtbar wäre (real aufgetreten: ein Klick nahe dem Bildrand statt dem tatsächlichen Kurvenzentrum ließ die Kurve weit über den Tisch hinausschießen). Eine spätere Zwischenversion leitete `dealer_area` stattdessen automatisch aus den bereits geklickten Sitz-Ecken ab (je Sitz die zwei zum Tisch-Zentroid nächstgelegenen Punkte) — auf einem länglichen ovalen Tisch mit ungleichmäßiger Klick-Dichte je Sitz ist "Abstand zum Zentroid" aber keine verlässliche Innen/Außen-Unterscheidung (verifiziert an echten Klick-Daten: eine echte Rail-Ecke eines Sitzes weit abseits der Längsachse kann näher am Zentroid liegen als die tatsächliche innere Ecke eines anderen Sitzes) und wurde deshalb wieder verworfen zugunsten des manuellen Nachzeichnens.
  - Operator klickt 4 Punkte für `board_zone`.
  - Die Homographie-Korrespondenzpunkte sind die 4 Ecken des Referenzfotos selbst (`(0,0)`, `(width,0)`, `(width,height)`, `(0,height)`), nicht geklickt — da Bildpixel = Tischkoordinate (s. u.), lösen beliebige ≥4 nicht-degenerierte Punkte exakt zur Identität auf.
  ...
  - `chip_zone` je Sitz: kein Rand zur Rail (Spieler stapeln Chips oft direkt am Rand auf), nur ein minimaler, konfigurierbarer Pixel-Rand (Default 10 px, `--chip-zone-inset-pixels`) zu Nachbarsitzen und zur Action Area. Umgesetzt als kantenweiser Versatz statt gleichmäßigem Schrumpfen Richtung Zentroid (frühere Variante — zieht auch die Rail-Kante nach innen, das Gegenteil des Ziels): je `player_area`-Kante wird per Skalarprodukt ihrer Auswärts-Normale mit der Sitz→Tisch-Zentrum-Richtung klassifiziert (Rail-Kante = Normale zeigt nah genug nach außen → 0 Rand; jede andere Kante → Inset), danach jeder Eckpunkt neu als Schnittpunkt seiner zwei (ggf. versetzten) Nachbarkanten berechnet ("mitred polygon offset"). Ein scharfer/spitzer Eckwinkel zwischen zwei Inset-Kanten kann diesen Schnittpunkt beliebig weit wegschießen lassen (bekannter Effekt, analog zum "Miter Join" bei Vektorgrafik-Strichen) — ab einer Grenz-Distanz greift stattdessen ein "Bevel"-Fallback (Mittelwert der beiden unabhängig versetzten Punkte). Ein davon unabhängiges, reales Problem in echten Klick-Daten: zwei versehentlich fast deckungsgleiche Klicks (Abstand wenige Pixel) an einem Sitz können auch mit diesem Fallback dazu führen, dass der Versatz eine entfernte, unabhängige Stelle desselben Polygons überschneidet — kein Geometrie-Bug, sondern ein Datenqualitätsproblem der Klick-Eingabe; das Tool bricht in diesem Fall mit einer klaren, auf den betroffenen Sitz verweisenden Fehlermeldung ab (Hinweis: Sitz neu nachzeichnen), statt die Kalibrierung still fehlerhaft zu erzeugen.
...
- AC-6a (REQ-10a): `calib mark-zones` erzeugt aus den geklickten/nachgezeichneten Punkten (10 Sitz-Polygone, ein markierter `card_dealer_seat_id`, eine frei nachgezeichnete `dealer_area`-Kontur, 4 Board-Zone-Punkte) eine REQ-11-valide `CalibrationAuthoring`; die Homographie-Korrespondenzpunkte werden aus den Bild-Ecken abgeleitet, nicht geklickt; Seat-Nummerierung ist im Uhrzeigersinn ab dem auf `card_dealer_seat_id` folgenden Sitz und deterministisch bei denselben Eingabepunkten; `chip_zone` hat keinen Rand zur Rail-Kante eines Sitzes und einen minimalen, konfigurierbaren Rand zu allen übrigen Kanten. Schlägt die abschließende REQ-11-Validierung bei `s` fehl (z. B. ein versehentlicher Doppel-Klick in einem Sitz-Polygon), bleibt das Fenster offen und zeigt den Fehler an, statt die Session kommentarlos zu verwerfen (real aufgetreten: das Fenster schloss sich sofort bei `s`, noch bevor validiert wurde) — der betroffene Sitz lässt sich per Klick, das innere Oval per Taste `o` gezielt neu nachzeichnen, ohne die übrigen bereits fertigen Sitze zu verlieren.
