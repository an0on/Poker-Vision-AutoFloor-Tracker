...
- REQ-10a: Interaktives Klick-Tool (`calib mark-zones`) zum Authoring der Referenzfoto-Geometrie, statt Koordinaten von Hand zu tippen oder algorithmisch zu raten (automatische Farbsegmentierung ist bei diesem Tischdesign nachweislich unzuverlässig — benachbarte gleichfarbige Sitze sind nur durch eine schwache Naht statt einer kontrastreichen Linie getrennt). Nimmt ein beliebiges Bild entgegen (Referenzfoto oder frisch aufgenommener Live-Frame — dasselbe Werkzeug für beide Fälle):
  ...
  - Operator zeichnet `dealer_area` (= Fläche innerhalb des inneren Ovals) als **Freihand-Spur** nach: beliebig viele Punkte entlang der tatsächlich sichtbaren inneren Randkurve, Enter/Space zum Abschließen — genau dieselbe Klick-Technik, mit der jeder Sitz seine eigene äußere Randkurve bereits nachzeichnet, kein Bogen-/Mittelpunkt-Konzept. **Vorgeschichte:** Das ursprüngliche Design dieses REQs sah ein parametrisiertes Oval vor (je 6 Punkte — Bogenanfang, Kreismittelpunkt, Bogenende — pro Ende); das erwies sich in der Praxis als fehleranfällig (ein leicht falsch platzierter Mittelpunkt-Klick erzeugt einen beliebig falschen Radius, real aufgetreten: die Kurve schoss weit über den Tisch hinaus). Ein Zwischenstand ersetzte das Klicken komplett durch Ableitung aus den `player_area`-Eckpunkten — geometrisch tragfähig, traf aber nicht die tatsächlich gewünschte Präzision der echten Randkurve; auf ausdrücklichen Wunsch des Repo-Owners daher zurückgenommen zugunsten der Freihand-Spur oben, die kein Bogen-Fehlermodus mehr hat (ein einzelner falscher Punkt verschiebt die Grenze nur geringfügig) und gleichzeitig die tatsächliche Kurve trifft.
  - Operator klickt 4 Punkte für `board_zone`.
  - Die Homographie-Korrespondenzpunkte sind die 4 Ecken des Referenzfotos selbst (`(0,0)`, `(width,0)`, `(width,height)`, `(0,height)`) statt eines geklickten äußeren Ovals — da Bildpixel = Tischkoordinate (s. u.), lösen beliebige ≥4 nicht-degenerierte Punkte exakt zur Identität auf; ein äußeres Oval hätte dafür ohnehin nichts beigetragen, das nicht schon diese 4 Punkte leisten (diese Vereinfachung bleibt bestehen, unabhängig vom inneren Oval oben).
  - Das Tool leitet zusätzlich ab: Seat-Nummerierung `seat_1..seat_10` im Uhrzeigersinn beginnend beim Sitz direkt nach dem als `card_dealer_seat_id` markierten Sitz, welcher damit selbst immer `seat_10` erhält (auch im Standardfall neun Spieler + Kartengeber — ob an `seat_10` tatsächlich ein zehnter Spieler sitzt, ist Turnier-/Spielzustand, nicht Teil der Kalibrierung).
  - **`chip_zone` je Sitz:** ursprünglich ein konfigurierbarer Schrumpf-Faktor Richtung Zentroid (Default 50 %) auf `player_area` — das schrumpft aber die äußere, zur Rail zeigende Kante mit, obwohl die meisten Spieler ihre Chips genau dort stapeln, direkt an der Rail (der Repo-Owner will die Erkennungsfläche maximieren, keine defensive Sicherheitsmarge). Stattdessen: `player_area`, geschnitten (Halbebenen-Clip, nicht Verschiebung einzelner Punkte) gegen eine Linie `--chip-zone-inset-pixels` (Default `DEFAULT_CHIP_ZONE_INSET_PIXELS = 10.0` — bewusst minimal, nur so viel Abstand wie nötig, um Mehrdeutigkeit zu vermeiden, keine Sicherheitsmarge; ein grober Platzhalter in Referenzfoto-Pixeln, keine gemessene Größe) hinter (a) dem Tisch-Zentroid (innere, zu `dealer_area` zeigende Kante) und (b) je einem der zwei direkten Sitz-Nachbarn (die beiden seitlichen Kanten) entfernt von deren jeweiligem Zentroid — die äußere, Rail-zugewandte Kante bleibt unverändert auf voller Ausdehnung. Eine Variante, die stattdessen einzelne "innere" Eckpunkte um einen festen Pixelwert Richtung Zentroid verschiebt, wurde zuerst versucht und funktioniert auf echten Klick-Daten nicht zuverlässig: bei einem unregelmäßigen, von Hand nachgezeichneten Polygon ist eine einzelne gemeinsame "Richtung" nicht für jeden Eckpunkt die eigene lokale Innennormale, wodurch ein Punkt quer durch eine benachbarte Kante des eigenen Polygons hinausgeschoben werden kann. Ein Geraden-Clip kann dagegen nur Fläche entfernen, nie einen Punkt aus dem Ausgangspolygon herausbewegen, unabhängig davon wie unregelmäßig das Polygon ist. Der Clip darf außerdem nur auf Punkten der eigenen Innenseite eines Sitzes verankert werden (nicht auf einer beliebigen, dem Nachbarn am nächsten liegenden Ecke), sonst könnte er versehentlich in eine äußere, Rail-zugewandte Ecke einschneiden. Zwei zusätzliche, an den echten 10 Sitzen bestätigte Sicherheitsmechanismen: (1) reicht der angeforderte Inset für die Innenkanten-Klippung eines bestimmten, konkav/unregelmäßig geklickten Sitzes nicht (bricht REQ-11s Containment), werden alle Insets dieses einen Sitzes (Innenkante und beide Nachbarn gemeinsam) automatisch skaliert und erneut geprüft, bis sie passen; (2) überlappen sich zwei benachbarte `chip_zone`s trotzdem noch (reale, an den echten Klicks bestätigte Ursache: zwei unabhängig von Hand geklickte `player_area`-Polygone überlappten sich bereits selbst etwas an ihrer gemeinsamen Grenze, nicht nur ein Rundungsfehler), wird der Inset für genau dieses Nachbarpaar verdoppelt und neu berechnet, bis die Überlappung verschwindet oder ein Rundenlimit erreicht ist (dann greift REQ-11s reguläre Fehlermeldung). Die Reihenfolge, in der die beiden Nachbarn eines Sitzes geklippt werden, ist deterministisch fest (nicht von Python-Set-Iterationsreihenfolge/Hash-Randomisierung abhängig), damit dieselbe Eingabe über verschiedene Prozessläufe hinweg byte-identisch bleibt (AC-6).
...
- AC-6a (REQ-10a): `calib mark-zones` erzeugt aus den geklickten Punkten (10 Sitz-Polygone, ein markierter `card_dealer_seat_id`, eine Freihand-`dealer_area`-Spur, 4 Board-Zone-Punkte) eine REQ-11-valide `CalibrationAuthoring`; `dealer_area` ist exakt die geklickte Spur, die Homographie-Korrespondenzpunkte werden aus den Bild-Ecken abgeleitet, nicht geklickt; Seat-Nummerierung ist im Uhrzeigersinn ab dem auf `card_dealer_seat_id` folgenden Sitz und deterministisch bei denselben Eingabepunkten; `chip_zone`s äußere (Rail-zugewandte) Kante bleibt gegenüber `player_area` unverändert; verifiziert an der tatsächlichen 10-Sitz-Klick-Sitzung des Repo-Owners (nicht nur an synthetischen Test-Fixtures), inklusive der beiden Fälle, in denen der Standard-Inset für einen einzelnen konkaven Sitz bzw. ein einzelnes Nachbarpaar automatisch angepasst werden musste.
