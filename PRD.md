...
- REQ-10a: Interaktives Klick-Tool (`calib mark-zones`) zum Authoring der Referenzfoto-Geometrie, statt Koordinaten von Hand zu tippen oder algorithmisch zu raten (automatische Farbsegmentierung ist bei diesem Tischdesign nachweislich unzuverlässig — benachbarte gleichfarbige Sitze sind nur durch eine schwache Naht statt einer kontrastreichen Linie getrennt). Nimmt ein beliebiges Bild entgegen (Referenzfoto oder frisch aufgenommener Live-Frame — dasselbe Werkzeug für beide Fälle):
  ...
  - **Kein separates Klicken von innerem/äußerem Oval mehr** (ursprüngliches Design dieses REQs: je 6 Punkte — Bogenanfang, Kreismittelpunkt, Bogenende — pro Oval). In der Praxis erwies sich der Kreismittelpunkt-Klick als fehleranfällig: ein leicht falsch platzierter Mittelpunkt erzeugt einen beliebig falschen Radius, ohne dass der Fehler vor dem Speichern sichtbar wäre (real aufgetreten: ein Klick nahe dem Bildrand statt dem tatsächlichen Kurvenzentrum ließ die Kurve weit über den Tisch hinausschießen). Ersetzt durch Ableitung aus bereits geklickten, geprüften Punkten:
    - `dealer_area` (= Fläche innerhalb des inneren Ovals) wird aus den 10 `player_area`-Polygonen abgeleitet: je Sitz **ein** zum Tisch-Zentroid nächstgelegener Eckpunkt (nicht zwei — siehe unten), alle 10 davon nach Winkel um den Zentroid sortiert ergeben ein einfaches, dem inneren Rand folgendes Polygon (schließt `chip_zone`/`player_area` per Konstruktion aus, da deren übrige Punkte weiter außen liegen). **Korrektur nach Verifikation an den tatsächlichen 10 Sitz-Klicks des Repo-Owners:** ursprünglich waren es die zwei nächstgelegenen Eckpunkte je Sitz; das bricht auf einem echten, länglich-ovalen Tisch mit unterschiedlicher Klick-Dichte je Sitz (5–18 Punkte, je nachdem wie viel der äußeren Randkurve mitgezeichnet wurde) — "Abstand zum einen gemeinsamen Zentroid" ist nicht monoton mit "wie weit Richtung Board", sobald ein Sitz entlang der langen Tischachse deutlich seitlich vom Zentroid liegt; ein echter Außen-Eckpunkt kann dann numerisch näher am Zentroid liegen als der eigene wahre Innen-Eckpunkt auf der anderen Seite. Der jeweils nächstgelegene (Rang-1-)Punkt war in den echten Daten durchgehend korrekt, der zweitnächste (Rang-2) nicht.
    - Die Homographie-Korrespondenzpunkte sind die 4 Ecken des Referenzfotos selbst (`(0,0)`, `(width,0)`, `(width,height)`, `(0,height)`) statt eines geklickten äußeren Ovals — da Bildpixel = Tischkoordinate (s. u.), lösen beliebige ≥4 nicht-degenerierte Punkte exakt zur Identität auf; ein äußeres Oval hätte dafür ohnehin nichts beigetragen, das nicht schon diese 4 Punkte leisten.
  ...
  - **`chip_zone` je Sitz (Korrektur, ebenfalls nach Verifikation an den echten Klicks):** ursprünglich ein konfigurierbarer Schrumpf-Faktor Richtung Zentroid (Default 50 %) auf `player_area` — das schrumpft aber die äußere, zur Rail zeigende Kante mit, obwohl die meisten Spieler ihre Chips genau dort stapeln, direkt an der Rail. Stattdessen: `player_area`, geschnitten (Halbebenen-Clip, nicht Verschiebung einzelner Punkte) gegen eine Linie `--chip-zone-inset-pixels` (Default `DEFAULT_CHIP_ZONE_INSET_PIXELS = 40.0`, ein grober Platzhalter für "etwa ein Feld des Filz-Karomusters" in Referenzfoto-Pixeln, keine gemessene Größe) hinter (a) dem Tisch-Zentroid (innere, zu `dealer_area` zeigende Kante) und (b) je einem der zwei direkten Sitz-Nachbarn (die beiden seitlichen Kanten) entfernt von deren jeweiligem Zentroid — die äußere, Rail-zugewandte Kante bleibt unverändert auf voller Ausdehnung. Eine Variante, die stattdessen einzelne "innere" Eckpunkte um einen festen Pixelwert Richtung Zentroid verschiebt, wurde zuerst versucht und funktioniert auf echten Klick-Daten nicht zuverlässig: bei einem unregelmäßigen, von Hand nachgezeichneten Polygon ist eine einzelne gemeinsame "Richtung" nicht für jeden Eckpunkt die eigene lokale Innennormale, wodurch ein Punkt quer durch eine benachbarte Kante des eigenen Polygons hinausgeschoben werden kann. Ein Geraden-Clip kann dagegen nur Fläche entfernen, nie einen Punkt aus dem Ausgangspolygon herausbewegen, unabhängig davon wie unregelmäßig das Polygon ist. Zwei zusätzliche, an den echten 10 Sitzen bestätigte Sicherheitsmechanismen: (1) reicht der angeforderte Inset für die Innenkanten-Klippung eines bestimmten, konkav/unregelmäßig geklickten Sitzes nicht (bricht REQ-11s Containment), wird der Inset für genau diesen Sitz automatisch halbiert und erneut geprüft, bis er passt; (2) überlappen sich zwei benachbarte `chip_zone`s trotzdem noch (reale, an den echten Klicks bestätigte Ursache: zwei unabhängig von Hand geklickte `player_area`-Polygone überlappten sich bereits selbst etwas an ihrer gemeinsamen Grenze, nicht nur ein Rundungsfehler), wird der Inset für genau dieses Nachbarpaar verdoppelt und neu berechnet, bis die Überlappung verschwindet oder ein Rundenlimit erreicht ist (dann greift REQ-11s reguläre Fehlermeldung).
...
- AC-6a (REQ-10a): `calib mark-zones` erzeugt aus den geklickten Punkten (10 Sitz-Polygone, ein markierter `card_dealer_seat_id`, 4 Board-Zone-Punkte) eine REQ-11-valide `CalibrationAuthoring`; `dealer_area` und die Homographie-Korrespondenzpunkte werden aus den Sitz-Polygonen bzw. den Bild-Ecken abgeleitet, nicht separat geklickt; Seat-Nummerierung ist im Uhrzeigersinn ab dem auf `card_dealer_seat_id` folgenden Sitz und deterministisch bei denselben Eingabepunkten; `chip_zone`s äußere (Rail-zugewandte) Kante bleibt gegenüber `player_area` unverändert; verifiziert an der tatsächlichen 10-Sitz-Klick-Sitzung des Repo-Owners (nicht nur an synthetischen Test-Fixtures), inklusive der beiden Fälle, in denen der Standard-Inset für einen einzelnen konkaven Sitz bzw. ein einzelnes Nachbarpaar automatisch angepasst werden musste.
